...
// DELETE /api/allocations/transfers/clear - Clear the history of transfer request queues (Admin only)
router.delete('/transfers/clear', authenticate, requireRole(['Admin']), async (req, res) => {
  try {
    const count = await TransferRequest.destroy({ where: {} });
    await logActivity({
      userId: req.user.id,
      action: 'CLEAR_TRANSFER_HISTORY',
      targetType: 'TransferRequest',
      targetId: null,
      details: `Admin cleared transfer request queue history. Deleted ${count} records.`,
    });
    return res.json({ message: `Successfully cleared ${count} transfer requests from history.` });
  } catch (error) {
    return res.status(500).json({ message: 'Failed to clear transfer history.', error: error.message });
  }
});
